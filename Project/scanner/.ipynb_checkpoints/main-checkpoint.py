...
class UnifiedScanner:
        ...
        try:
            def run_detection():
                ...
                if strategy == 'consolidation':
                    ...
                    for check_bar, is_current in [(-2, False), (-1, True)]:
                        if len(df) > 50 + abs(check_bar):
                            detected, result = detect_sma50_breakout(df, use_pre_breakout=False, check_bar=check_bar)
                            if detected:
                                results.append((result, is_current, check_bar))
                    ...
                    for check_bar, is_current in [(-2, False), (-1, True)]:
                        if len(df) > abs(check_bar) + 5:
                            ...
                            if len(df) > 57 + abs(check_bar):
                                sma50_detected, sma50_result = detect_sma50_breakout(df, check_bar=check_bar)
            ...
            if strategy == 'hbs_breakout':
                ...
                base_result.update({
                    ...
                    'sma50_breakout_strength': result_data['sma50_result'].get('breakout_strength', '') if result_data['sma50_detected'] else '',
    ...
    async def scan_market(self, symbol):
        ...
        pattern_strategies = {'consolidation', 'consolidation_breakout', 'channel', 'channel_breakout', 
                            'wedge_breakout', 'sma50_breakout', 'trend_breakout', 'pin_up', 'hbs_breakout'}
    ...
    async def send_telegram_message(self, strategy, results):
        """Send telegram messages with same formatting as original"""
        ...
        try:
            ...
            for result in results:
                ...
                # Format messages (keeping original telegram formatting logic)
                if strategy in vsa_strategies:
                    signal_message = (
                        f"Symbol: {symbol}\n"
                    ...
                    if result.get('has_sma50_breakout', False):
                        ...
                        sma_status = "Pre-Breakout" if sma50_type == "pre_breakout" else "Full Breakout" if sma50_type == "classic_breakout" else sma50_type.replace('_', ' ').title()
    ...
    async def scan_all_markets(self):
        """Scan all markets with optimized batching and parallel strategies"""
        try:
